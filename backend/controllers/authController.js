 ...
 *
 *  Patient routing logic in signup():
 *    - Adult (age >= 14)  → creates Person + Account + Patient (linked via personId)
 *    - Minor (age < 14)   → creates Children + Account + Patient (linked via childId)
 *      • parent must already exist as a Person
 *      • child gets auto-generated childRegistrationNumber (CRN-YYYYMMDD-XXXXX)
 *      • migrationStatus starts as 'pending'
 *
 *  Conventions kept from existing code:
 *    - Arabic error messages with descriptive emoji console logs
...
exports.signup = async (req, res) => {
  ...
  try {
    ...
    // ── Extract emergency contact (two formats) ──────────────────────────
    let emergencyName;
    ...
    // ── Create profile + account + patient ───────────────────────────────
    let profile;
    let account;
    let patient;
...
exports.registerDoctorRequest = async (req, res) => {
  ...
  try {
    ...
    if (checks[0] || checks[1]) {
      return res.status(400).json({
        success: false,
        message: 'الرقم الوطني مسجل مسبقاً في النظام'
      });
    }
    ...
    // ── Log what we're about to insert (helps diagnose schema failures) ──
    const insertPayload = {
      firstName: firstName.trim(),
      fatherName: fatherName.trim(),
      lastName: lastName.trim(),
    ...
    // ── Print detailed MongoDB schema validation failures ───────────────
    // MongoDB error 121 = document failed $jsonSchema validation. The actual
    // failing rules are buried in error.errInfo.details — surface them here
    // so we can see exactly which field violated the schema.
    ...
    // Also dump Mongoose validation errors with full per-field detail
    if (error.name === 'ValidationError') {
      console.error('🔍 ===== Mongoose validation errors =====');
      Object.entries(error.errors).forEach(([field, err]) => {
        console.error(`   ${field}: ${err.message} (value=${JSON.stringify(err.value)})`);
      });
      console.error('🔍 =====================================');
    ...
    if (error.code === 11000) {
      ...
      return res.status(400).json({
        success: false,
        message: `${arabicFields[field] || field} مسجل مسبقاً في النظام`
      });
    }
...
exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    if (!account) {
      console.log('⚠️  Email not found, returning generic success');
      return res.json({
        success: true,
